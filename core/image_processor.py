...
class ImageProcessor:
    ...
    def gaussian_blur(image, x, y, w, h, strength):
        ...
        if x < 0 or y < 0 or w <= 0 or h <= 0:
            raise ValueError(f"Invalid region coordinates: x={x}, y={y}, w={w}, h={h}")
        
        #  Ensure region is within image bounds
        if x >= img_w or y >= img_h:
            raise ValueError(f"Region starts outside image bounds")
        ...
        # Ensure blur strength is odd and valid
        if strength % 2 == 0:
            strength += 1
        ...
        try:
            ...
            raise ValueError(f"GaussianBlur failed: {str(e)}")
    
    @staticmethod
    def pixelate(image, x, y, w, h, pixel_size):
        """Apply pixelation effect with validation"""
        # Validate region
        if image is None or image.size == 0:
    ...
    def black_bar(image, x, y, w, h):
        """Apply solid black bar with validation"""
        if image is None or image.size == 0:
            raise ValueError("Image is empty")
        ...
        result = image.copy()
        cv2.rectangle(result, (x, y), (x_end, y_end), (0, 0, 0), -1)
        return result[y:y_end, x:x_end]
    
    @staticmethod
    def gradient_fade(image, x, y, w, h):
        """Apply gradient fade effect with validation"""
        if image is None or image.size == 0:
            raise ValueError("Image is empty")
        ...
        region = image[y:y_end, x:x_end].copy()
        ...
        # Apply gradient
        blur_kernel = min(99, h_reg - 1 if h_reg % 2 == 0 else h_reg)
        blur_kernel = max(3, blur_kernel)
        if blur_kernel % 2 == 0:
            blur_kernel += 1
        
        blurred = cv2.GaussianBlur(region, (blur_kernel, blur_kernel), 0)
        for c in range(3):
    ...
    def mosaic_effect(image, x, y, w, h, tile_size=10):
        """Apply mosaic effect with validation"""
        if image is None or image.size == 0:
            raise ValueError("Image is empty")
    ...
    def frosted_glass(image, x, y, w, h, strength=15):
        """Apply frosted glass effect with validation"""
        if image is None or image.size == 0:
            raise ValueError("Image is empty")
    ...
    def oil_paint(image, x, y, w, h, size=7, dynRatio=1):
        """Apply oil painting effect with validation"""
        if image is None or image.size == 0:
            raise ValueError("Image is empty")
    ...
    def emoji_cover(image, x, y, w, h, emoji_char='😎'):
        """Place emoji over region with validation"""
        if image is None or image.size == 0:
            raise ValueError("Image is empty")
        ...
        emoji_size = min(x_end - x, y_end - y)
        ...
        # Draw emoji
        text_x = x + ((x_end - x) - emoji_size) // 2
        text_y = y + ((y_end - y) - emoji_size) // 2
