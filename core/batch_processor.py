...
from src.config import SUPPORTED_FORMATS, BLUR_RANGE, PIXEL_RANGE, OPACITY_RANGE
...
class BatchProcessor:
    ...
    def process_batch(self, input_paths: List[str], output_dir: str,
                     ...
                     is_cancelled: Callable[[], bool] = None) -> List[Dict]:
        """
        ...
        Args:
            ...
            is_cancelled: Optional function returning True if process should stop
    ...
    def _process_single_image(self, input_path: str, output_dir: str, settings: Dict) -> Dict:
        ...
        is_pro = settings.get('is_pro', False)
        effect_params = settings.get('effect_params', {})

        # Detect regions
        regions = self._detect_regions(image, detection_mode, is_pro)
        ...
        # Check if the returned regions are PRO AI regions (list of dicts)
        # or FREE regions (list of tuples/lists)
        is_ai_regions = isinstance(regions[0], dict) if len(regions) > 0 else False

        if is_ai_regions:
            # PRO AI regions: polygon or rotated_rect
            effect_kwargs = dict(
                strength   = effect_params.get('blur_strength', BLUR_RANGE['default']),
                pixel_size = effect_params.get('pixel_size', PIXEL_RANGE['default']),
                opacity    = effect_params.get('opacity', OPACITY_RANGE['default']),
            )
            for det in regions:
                if det["type"] == "polygon":
                    processed_image = self.image_processor.apply_effect_to_polygon(
                        processed_image, det["points"], effect_type, **effect_kwargs
                    )
                elif det["type"] == "rotated_rect":
                    processed_image = self.image_processor.apply_effect_rotated(
                        processed_image,
                        det["center"], det["size"], det["angle"],
                        effect_type, **effect_kwargs
                    )
        else:
            # FREE rectangle regions
            for (x, y, w, h) in regions:
                processed_image = self._apply_effect_to_region(
                    processed_image, x, y, w, h, effect_type, effect_params
                )

        # Save processed image (Unicode safe)
        output_filename = f"processed_{Path(input_path).stem}.jpg"
        output_path = os.path.join(output_dir, output_filename)
    ...
    def _detect_regions(self, image, detection_mode: str, is_pro: bool = False) -> List:
        """Detect regions based on the specified mode"""
        if detection_mode == 'face':
            if is_pro:
                res = self.detection_engine.detect_faces_pro(image)
                if res:
                    return res
            return self.detection_engine.detect_faces(image)
        elif detection_mode == 'eye':
            if is_pro:
                res = self.detection_engine.detect_eyes_pro(image)
                if res:
                    return res
    ...
    def _apply_effect_to_region(self, image, x: int, y: int, w: int, h: int,
        ...
        """Apply the specified effect to a region using the unified ImageProcessor method"""
        strength = effect_params.get('blur_strength', BLUR_RANGE['default'])
        pixel_size = effect_params.get('pixel_size', PIXEL_RANGE['default'])
        opacity = effect_params.get('opacity', OPACITY_RANGE['default'])
        
        return self.image_processor.apply_effect_to_region(
            image, x, y, w, h, effect_type,
            strength=strength, pixel_size=pixel_size, opacity=opacity
        )
    ...
    def process_batch_async(self, input_paths: List[str], output_dir: str,
        ...
        def worker():
            self.process_batch(input_paths, output_dir, settings,
                             progress_callback, error_callback, is_cancelled)
    ...
    def validate_input_files(self, file_paths: List[str]) -> Tuple[List[str], List[str]]:
        ...
        for path in file_paths:
            ...
            # Try to load image to verify it's valid (Unicode safe)
            try:
